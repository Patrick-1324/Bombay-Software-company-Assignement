...
def train_and_save_model(features, labels, model_path):
    ...
    train_accuracy = accuracy_score(y_train, knn.predict(X_train))
    test_accuracy = accuracy_score(y_test, knn.predict(X_test))
    print(f"KNN with k=11 - Training Accuracy: {train_accuracy:.4f}")
    print(f"KNN with k=11 - Test Accuracy: {test_accuracy:.4f}")
...
model_path = 'knn_model_set3(02:45).pkl'
print("Training and saving the model for Feature Set 1:")
